...
public class SimpleExample {
	...
	public void run(long a_maxReads, String a_topic, int a_partition,
		...
		//
		PartitionMetadata metadata = KafkaUtils.findLeader(a_seedBrokers,
		...
		while (a_maxReads > 0) {
			...
			FetchRequest req = new FetchRequestBuilder().clientId(clientName)
					.addFetch(a_topic, a_partition, 575, 100000) // Note:
																		// this
																		// fetchSize
																		// of
																		// 100000
	...
	private String findNewLeader(String a_oldLeader, String a_topic,
		...
		for (int i = 0; i < 3; i++) {
			...
			PartitionMetadata metadata = KafkaUtils.findLeader(
					m_replicaBrokers, a_port, a_topic, a_partition);
			if (metadata == null) {
				goToSleep = true;
